...
from app.queries.timelines import DELETE_APPLICATION_TIMELINE, GET_APPLICATION_TIMELINE, INSERT_APPLICATION_TIMELINE, UPDATE_APPLICATION_TIMELINE
from fastapi import APIRouter, HTTPException, Depends
from typing import List
from app.database import get_db_connection
from app.queries.applications import DELETE_JOB_APPLICATION, GET_JOB_APPLICATION_BY_ID, GET_JOB_APPLICATIONS, INSERT_JOB_APPLICATION, UPDATE_JOB_APPLICATION
...
router = APIRouter(prefix="/bpi", tags=["Applications"])
...
async def get_job_application(job_id: int, current_user: str = Depends(get_current_user)):
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                (current_user, job.company, job.position, job.status, job.date,
                 job.priority, job.matched_skills, job.required_skills)
            )
...
async def update_job_application(
    job_id: int, job_update: JobApplicationCreate, current_user: str = Depends(get_current_user)
):
    """Update a job application"""
    with get_db_connection() as conn:
        with conn.cursor() as cur:

            cur.execute("SELECT * FROM job_applications WHERE id = %s AND user_email = %s",
                        (job_id, current_user))
            ...
            cur.execute(
                ...
                (
                    job_update.company, job_update.position, job_update.status, job_update.date,
                    job_update.priority, job_update.matched_skills, job_update.required_skills,
                    job_id, current_user
                )
            )
...
async def delete_job_application(job_id: int, current_user: str = Depends(get_current_user)):
...
@router.get("/applications/{job_id}/timeline", response_model=List[ApplicationTimelineResponse])
async def get_application_timeline(job_id: int, current_user: str = Depends(get_current_user)):
...
@router.post("/applications/{job_id}/timeline", response_model=ApplicationTimelineResponse)
async def add_timeline_entry(
    job_id: int, timeline_entry: ApplicationTimelineCreate, current_user: str = Depends(get_current_user)
):
...
@router.put("/applications/{job_id}/timeline/{timeline_id}", response_model=ApplicationTimelineResponse)
async def update_timeline_entry(
    job_id: int, timeline_id: int, timeline_update: ApplicationTimelineCreate, current_user: str = Depends(get_current_user)
):
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                (timeline_update.status, timeline_update.date,
                 timeline_update.notes, timeline_id, current_user)
            )
...
async def delete_timeline_entry(job_id: int, timeline_id: int, current_user: str = Depends(get_current_user)):
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(DELETE_APPLICATION_TIMELINE,
                        (timeline_id, current_user))
