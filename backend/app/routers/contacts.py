...
from app.queries.contacts import DELETE_CONTACT, GET_ALL_CONTACTS, GET_CONTACT_BY_ID, INSERT_CONTACT, UPDATE_CONTACT
from app.models.contacts import NetworkContactCreate, NetworkContactResponse

router = APIRouter(prefix="/bpi", tags=["Contacts"])
...
async def create_contact(
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                INSERT_CONTACT,
                (current_user, contact.name, contact.role, contact.company,
...
async def update_contact(
    contact_id: int, contact_update: NetworkContactCreate, current_user: str = Depends(get_current_user)
):
    ...
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                UPDATE_CONTACT,
                (contact_update.name, contact_update.role, contact_update.company,
                 contact_update.linkedin, contact_update.email, contact_update.phone,
...
async def delete_contact(contact_id: int, current_user: str = Depends(get_current_user)):
    """Delete a contact"""
    with get_db_connection() as conn:
        with conn.cursor() as cur:
